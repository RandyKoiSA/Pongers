...
class SidePlayer(Player_Base):
    ...
    def __init__(self, screen, controller, settings, bRightSide, player_type, width = 5, height = 100, color = (0, 0, 0), velocity=Vector(20,20)):
        """ Initialize properties """
        super().__init__(screen, controller, settings, player_type, width, height, color, velocity)
        self.bRightSide = bRightSide

        if not bRightSide:
            self.rect = pygame.Rect((15, self.settings.WINDOW_HEIGHT / 2 + self.player_height),
                                    (self.player_width, self.player_height))
        else:
            self.rect = pygame.Rect((self.settings.WINDOW_WIDTH - 25, ((self.settings.WINDOW_HEIGHT / 2) + self.player_height)),
                                    (self.player_width, self.player_height))
